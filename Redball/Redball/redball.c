...
typedef enum GameScreen {GAMEPLAY, PAUSE, MAIN_MENU, LOGO, HOW_TO_PLAY, CREDITS,ENDING,CLEAR,LEVELS, BALL_ANIMATION} GameScreen;
...
int main(void)
{
   ...
   //Box A: Moving box (level 1)
    ...
    int starsEarnedLevel1 = 0;
    int starsEarnedLevel2 = 0;
    int starsEarnedLevel3 = 0;
    int Score = 0; 
    int maxStars = 9;
    ...
    Player player = { 0 };
    player.position = (Vector2){ 250, 1370 }; //Player starting position 
    player.speed = 0;
    ...
    Vector2 ballPosition = { screenWidth / 2.0f, -20.0f }; 
    ...
    int bounceCount = 0; 
    bool growBall = false; 
    float growSpeed = 4.5f; // Growth rate
    ...
    while (!WindowShouldClose())
    {
        ...
        framesCounter++;
            ...
            alpha -= 0.02f;
                if (alpha <= 0.0f) {
                alpha = 0.0f;
                // Transition directly to BALL_ANIMATION after logo animation ends
                currentScreen = BALL_ANIMATION;
                }
            }
        }
    ...
    case BALL_ANIMATION: {
     ...
     if (!growBall) {
            ...
            // Collusion and jumping
            if (ballPosition.y + ballRadius >= screenHeight / 2.0f) {
                ballPosition.y = screenHeight / 2.0f - ballRadius; 
            ...
            ballRadius += growSpeed;


        
        if (ballRadius >= GetScreenWidth() - 480) {
         ...
         // moving obstacle for level 2
          ...
          if((player.position.x - boxH.x > -15)&&(player.position.x - boxH.x < 60))
                     ...
                     if (((boxH.x + boxH.width) >= GetScreenWidth()+12800) || (boxH.x <= 13800)) boxHSpeedX *= -1;
                        ...
                        if ((player.position.x >= 14570) && (player.position.x <= 14580) && (player.position.y <=530) && (player.position.y >=430))
                        {
                            PlaySound(winS);                              
                            Level = 4;
                            currentScreen = CLEAR;
                        ...
                        if(Level == 1)
                        {
                            ...
                            for (int i = 0; i < 3; i++)
                            {
                                if (i < starsEarnedLevel1)
                                {
                                DrawCircle(player.position.x + i * 55 - 520, player.position.y - 200, 22, RED); // Draw purple ball
                                } else {
                        ...
                        totalStarsEarned = starsEarnedLevel1 + starsEarnedLevel2 + starsEarnedLevel3;
            ...
            {   
                ...
                if(Level == 4)
                {   
                    DrawText("All Levels are Cleared, Congrats!", 300, 150, 60, WHITE);
                    DrawText(TextFormat("Your Score is %d/%d", Score, maxStars), 600, 250, 30, WHITE);
                    DrawText("Press Enter to Continue", 550, 350, 30, RED);
            ...
            case CREDITS:
            {
                ...
                DrawText("Backgrounds, Objects and Sounds",450,100,30,GRAY);
                ...
                DrawText("Press space to return to main menu",1200,620,20,GRAY);
                DrawText("https://gfxsounds.com/free-sound-effects/",500,350,30,GRAY);
                DrawText("Resources;",450,400,30,GRAY);
                DrawText("https://www.raylib.com/examples.html",500,450,30,GRAY);
